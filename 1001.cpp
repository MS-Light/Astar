...
//   proj
//
//  Created by YANBO JIANG on 7/11/20.
...
void showPath(stack<pair<short int,pair<short int, short int> > > path){
    ...
    cout<<endl;
    for (short int i = 0; i< 901; i++){
        for (short int j = 0; j<901; j++){
            cout << arr2[i][j]<<" ";
        }
        cout<<endl;
    }
...
int distance2(priority_queue<pair<pair<short int,short int>, pair<short int, short int> > > queue2){
    ...
    while(!queue2.empty()){
        ...
        if ((graph[a][b+1] == 0 ||graph[a][b+1]>dist+1) && b < 900){
            graph[a][b+1] = dist+1;
            queue2.emplace(make_pair(make_pair(-h[a][b+1]-graph[a][b+1],graph[a][b+1]),make_pair(a,b+1)));;
        }
        if ((graph[a+1][b] == 0 ||graph[a+1][b]>dist+1) && a < 900){
            graph[a+1][b] = dist+1;
...
void distance(priority_queue<pair<pair<short int,short int>, pair<short int, short int> > > queue1){
    while(!queue1.empty()){
        ...
        if ((arr[a][b+1] == 0 ||arr[a][b+1]>dist+1) && b < 900){
            arr[a][b+1] = dist+1;
        ...
        if ((arr[a+1][b] == 0 ||arr[a+1][b]>dist+1) && a < 900){
            arr[a+1][b] = dist+1;
